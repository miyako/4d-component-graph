...
<xsl:param name="v" />
	<xsl:text>
		tooltips: {
			...
			<xsl:for-each select="/graph/x_values/v">
				<xsl:variable name="position" select="position()" />
				...
				<xsl:for-each select="/graph/legend/v">
					<xsl:variable name="item" select="position()" />
					<xsl:variable name="x_value" select="/graph/legend/v[position() = $item]" />
					<xsl:variable name="y_value" select="/graph/y_values[position() = $item]/s[position() = $position]" />						
					<xsl:if test="$item != 1"><xsl:text>, </xsl:text></xsl:if>
					<xsl:call-template name="escape-string">
...
<xsl:param name="total" />
	<xsl:text>
		values: {
			...
			<xsl:for-each select="$v">				
				<xsl:variable name="position" select="position()" />
				<xsl:variable name="value" select="sum(/graph/y_values[position() = $position]/v)" />
				<xsl:if test="$position != 1"><xsl:text>, </xsl:text></xsl:if>
	...
	<xsl:text>
		values: {
			...
			<xsl:for-each select="$v">
				...
				<xsl:if test="$position != 1"><xsl:text>, </xsl:text></xsl:if>							
				<xsl:value-of select="concat('serie', $position, ': [')" />
				<xsl:variable name="total" select="sum(/graph/y_values[position() = $position]/v)" />					
				<xsl:for-each select="/graph/y_values[position() = $position]/v">
					<xsl:variable name="item" select="position()" />
		...
		tooltips: {
			...
			<xsl:for-each select="$v">
				...
				<xsl:for-each select="/graph/y_values[position() = $position]/v">
					...
					<xsl:variable name="value" select="." />
					<xsl:variable name="percentage" select="concat(floor(($value div $total) * 100), '%')" />
					<xsl:variable name="legend" select="/graph/legend/v[position() = $position]" />					
						<xsl:if test="$item != 1"><xsl:text>, </xsl:text></xsl:if>
						<xsl:call-template name="escape-string">
							<xsl:with-param name="s" select="concat('&lt;b&gt;', $legend, '&lt;/b&gt;', '&lt;br /&gt;' ,$value, ' : ', $percentage)"/>
						</xsl:call-template>						
