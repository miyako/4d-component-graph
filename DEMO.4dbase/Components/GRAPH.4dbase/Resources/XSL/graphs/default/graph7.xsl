...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:import href="common/graph_settings.xsl" />
	...
	<xsl:template match="graph">
		...
		<!-- legend properties -->
		<xsl:variable name="legend_max_car" select="exsl4D:max-string-length(legend_pie/v)+2"/>
		<!--<xsl:variable name="legend_max_car" select="10+2"/>-->
		<xsl:variable name="legend_label_width" select="$legend_max_car*$legend_font_width"/>
		<xsl:variable name="legend_box_width" select="number(boolean(legend_pie/v))*($base_margin+$legend_bullet_size+$base_margin+$legend_label_width+$base_margin)"/>
		<xsl:variable name="legend_box_height" select="$base_margin+(($legend_bullet_size+$base_margin)*count(legend_pie/v))"/>

		<xsl:variable name="y_values_count" select="count(y_values)" />

		<!-- graph margins -->
		<xsl:variable name="graph_left_margin" select="$base_margin*2" />
		<xsl:variable name="graph_right_margin" select="(($legend_box_width+$base_margin)*number(boolean(legend_pie/v)))+$base_margin"/>
				...
				<xsl:if test="legend_pie/v" >
					<g id="ID_legend" filter="{$legend_rect_filter}">
						...
						<g id="ID_legend_bullets" stroke="NONE">
							<xsl:for-each select="legend_pie/v">
