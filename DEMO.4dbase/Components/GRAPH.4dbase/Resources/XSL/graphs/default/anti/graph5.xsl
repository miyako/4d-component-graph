...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="graph">
    ...
    <xsl:variable name="graph_right_margin" select="exsl4D:max((($legend_box_width+$base_margin)*number(boolean(x_values/v)))+$base_margin,$base_margin+($x_label_width div 2))"/>
      ...
      <g id="ID_globals" style="fill:{$font_color};stroke:none;font-family:{$font_family};font-size:{$font_size}px;font-weight:{$font_weight};font-style:{$font_style};text-decoration:{$font_decoration}">
        ...
        <xsl:for-each select="settings/x_grid">
          <xsl:if test="text() = 'true'" >
            <xsl:variable name="count_v" select="count(../../legend/v)" />
            <g id="ID_x_grid" stroke="{$x_grid_color}" stroke-width="{$x_grid_width}">
              <xsl:for-each select="../../legend/v[position() &lt; $count_v]">
		...
		<xsl:variable name = "position" select="position()" />
            ...
            <polyline id="{concat('ID_polyline_', position(), '#legend', position(), '_color')}" fill="url(#ID_bar_grad{position()})" points="{$points}"/>
          </xsl:for-each>
