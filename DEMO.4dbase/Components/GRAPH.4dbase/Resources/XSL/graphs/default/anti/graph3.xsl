...
<xsl:key name="s_at_position" match="/graph[1]/y_values/s" use="count(preceding-sibling::s)" />
<xsl:key name="y_values_at_position" match="/graph[1]/y_values" use="count(preceding-sibling::y_values)" />
  ...
  <xsl:template match="graph">
    ...
    <xsl:variable name="y_label_max_car" select="exsl4D:max-value-length(y_grid_values_anti/v,$y_label_format_number,$y_label_format_date,$y_label_format_time)"/>
    <xsl:variable name="y_label_width" select="$y_label_font_width*$y_label_max_car"/>
    <xsl:variable name="y_label_count" select="count(y_grid_values_anti/v)" />
...
<!--<xsl:variable name="y_max" select="number(settings/y_max/@stack)"/>-->
	<xsl:variable name="y_max">
		<xsl:call-template name="max" />
	</xsl:variable> 
		
<!--<xsl:variable name="y_min" select="number(settings/y_min/@stack)"/>-->
        ...
        <xsl:for-each select="settings/y_grid">
          <xsl:if test="text() = 'true'" >
            <g id="ID_y_grid" stroke="{$y_grid_color}" stroke-width="{$y_grid_width}">
              <xsl:for-each select="../../y_grid_values_anti/s">
              ...
              <g fill="url(#ID_bar_grad{position()})" stroke="none">
                ...
                <xsl:choose>
                  <xsl:when test="number(text()) &gt; 0">
                   <xsl:variable name="bar_height" select="exsl4D:sum-pos(//y_values[$xpos]/s[position() &lt; ($y_values_index+1)])*$y_scale" />
                  ...
                  <xsl:otherwise>
                    <xsl:variable name="bar_height" select="math:abs(exsl4D:sum-neg(//y_values[$xpos]/s[position() &lt; ($y_values_index+1)]))*$y_scale" />
