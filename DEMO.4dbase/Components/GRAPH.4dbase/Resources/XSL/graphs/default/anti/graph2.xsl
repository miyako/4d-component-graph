...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="graph">
    ...
    <xsl:variable name="y_label_max_car" select="exsl4D:max-value-length(y_grid_values_relative/v,$y_label_format_number,$y_label_format_date,$y_label_format_time)"/>
    <xsl:variable name="y_label_width" select="$y_label_font_width*$y_label_max_car"/>
    <xsl:variable name="y_label_count" select="count(y_grid_values_relative_anti/v)" />
    ...
    <xsl:variable name="y_max" select="number(settings/y_max/@anti)"/>
    <xsl:variable name="y_min" select="number(settings/y_min/@anti)"/>
        ...
        <xsl:for-each select="settings/y_grid">
          <xsl:if test="text() = 'true'" >
            <g id="ID_y_grid" stroke="{$y_grid_color}" stroke-width="{$y_grid_width}">
              <xsl:for-each select="../../y_grid_values_relative/s">
	...
	<!--<xsl:for-each select="//y_values[1]/s">-->
		<xsl:for-each select="y_values">
          ...
          <g id="ID_bars_{position()}" stroke="none">
            <xsl:for-each select="//y_values[$xpos]/s">
              <g fill="url(#ID_bar_grad{position()})" stroke="none">
                ...
                <!-- the rectangle -->
                <xsl:choose>
                  <xsl:when test="number(@relative) &gt; 0">
				<!--<xsl:variable name="bar_height" select="exsl4D:sum-pos(//y_values[position() &lt; ($y_values_index+1)]/s[$xpos]/@relative)*$y_scale" />-->
				   <xsl:variable name="bar_height" select="exsl4D:sum-pos(//y_values[$xpos]/s[position() &lt; ($y_values_index+1)]/@anti)*$y_scale" />
				   
                   <xsl:variable name="yt"        select="$y0 - $bar_height" />
               <!--<xsl:variable name="yb"        select="$yt + (number(@relative)*$y_scale)" />-->
			   <xsl:variable name="yb"        select="$yt + (number(@anti)*$y_scale)" />
                   ...
                   <xsl:if test="($ybottom - $ytop) &gt; 0" >
                     <xsl:call-template name="bar_1">
						...
						<xsl:with-param name="id" select="concat('ID_bars_', $xpos, '#', 'legend', position() ,'_color')" />
                     </xsl:call-template>
                   </xsl:if>
                  </xsl:when>
                  <xsl:otherwise>
                <!--<xsl:variable name="bar_height" select="math:abs(exsl4D:sum-neg(//y_values[position() &lt; ($y_values_index+1)]/s[$xpos]/@relative))*$y_scale" />-->
					<xsl:variable name="bar_height" select="math:abs(exsl4D:sum-neg(//y_values[$xpos]/s[position() &lt; ($y_values_index+1)]/@relative))*$y_scale" />
                    <xsl:variable name="yb"        select="$y0 + $bar_height" />
                 <!--<xsl:variable name="yt"        select="$yb + (number(@relative)*$y_scale)" />-->
				 <xsl:variable name="yt"        select="$yb + (number(@anti)*$y_scale)" />
                    <xsl:variable name="ytop"      select="exsl4D:max(exsl4D:min($yt, $ymax), $ymin)" />
                    <xsl:variable name="ybottom"   select="exsl4D:max(exsl4D:min($yb, $ymax), $ymin)" />
                    <xsl:if test="($ybottom - $ytop) &gt; 0" >
                      <xsl:call-template name="bar_1">
